 ...
 * requests are IP-allowlisted on FatSecret's side; the app's consumer
 * key/secret double as the OAuth2 client id/secret.
 */
...
export class OAuth2ClientCredentials {
  ...
  constructor(options: OAuth2ClientOptions = {}) {
    this.clientId = options.clientId ?? process.env.FATSECRET_CONSUMER_KEY ?? '';
    this.clientSecret = options.clientSecret ?? process.env.FATSECRET_CONSUMER_SECRET ?? '';
